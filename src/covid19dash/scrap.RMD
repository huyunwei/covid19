...
plotly::ggplotly(apple_mobility %>% 
...
```
```{r}
plotly::ggplotly(google_mobility %>% 
  filter(country_region_code == "US",
         sub_region_1 == "Texas",
         sub_region_2 == "Harris County") %>% 
  ggplot(aes(x = date, y = pct_diff, group = type, color = type)) +
  geom_point(size = 0.5, alpha = 0.5) +
  # scale_color_manual(values = my.colors("bly")) +
...
```{r}
test <- metro_ts %>% 
  filter(date %in% c(ymd(20200424), ymd(20200423))) %>% 
  filter(metro == "New York-Newark, NY-NJ-CT-PA") 

## JHU missing NYC, NY 4/24 data in GitHub repo as of 20200425
```
```{r}
...
```{r}
jhu_ts <- download_jhu_csse_covid19_data()
apple_mob <- download_apple_mtr_data()
apple_mob_city <- download_apple_mtr_data(type = "country_city")
jhu_ts_us <- download_jhu_csse_covid19_data(type = "us_county")
```
